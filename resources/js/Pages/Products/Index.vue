    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div class="min-w-0">
                    <h2 class="text-2xl font-semibold text-slate-900">{{ $t('products.title') }}</h2>
                    <p class="mt-1 text-sm text-slate-600">{{ $t('products.description') }}</p>
                </div>
            ...
            <div class="mx-auto max-w-7xl space-y-4 sm:px-6 lg:px-8">
                <div class="rounded-xl border border-teal-100 bg-white/90 shadow-sm shadow-teal-100/50">
                    <div v-if="isEmpty" class="px-6 py-16 text-center">
                        ...
                        <h3 class="mt-3 text-base font-semibold text-slate-900">{{ isSearchActive ? $t('products.no_matching') : $t('products.no_products_yet') }}</h3>
                            {{ isSearchActive ? $t('products.try_broader') : $t('products.create_first') }}
                        <Link
                            v-if="!isSearchActive"
                            :href="route('products.create')"
                            ...
                            <table class="min-w-full border-collapse">
                                <thead>
                                    <tr class="bg-gradient-to-r from-teal-50/70 to-cyan-50/60">
                                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-teal-700/80">{{ $t('products.product') }}</th>
                                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-teal-700/80">{{ $t('products.ean') }}</th>
                                        <th v-if="isVatPayer" class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-teal-700/80">{{ $t('products.vat') }}</th>
                                        <th class="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-teal-700/80">{{ $t('products.price') }}</th>
                                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-teal-700/80">{{ $t('products.status') }}</th>
                                        <th class="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-teal-700/80">{{ $t('products.actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr
                                    ...
                                    >
                                        ...
                                        <td class="px-5 py-4 text-right align-top">
                                            ...
                                            <p v-if="isVatPayer" class="mt-0.5 text-xs text-slate-500">{{ $t('products.incl_vat') }}</p>
                                        </td>
                                        <td class="px-5 py-4 align-top">
                                            <span
                            ...
                            <article v-for="product in products.data" :key="product.id" class="rounded-lg border border-slate-200 p-4">
                                ...
                                <div class="mt-3 grid grid-cols-2 gap-3 text-xs text-slate-600">
                                    ...
                                    <div :class="isVatPayer ? '' : 'col-span-2'">
                                        <p class="text-slate-500">{{ $t('products.price') }}</p>
